...
const { parseUnits , formatBytes32String, parseBytes32String, defaultAbiCoder} = require("ethers/lib/utils");
...
describe("Droppin", async () => {
  describe("Core", async ()=>{
    ...
    it("should be able to add 1 or more condition to group", async () => {
      ...
      let rd = await cDroppin.conditionById(3);
    ...
    it("FAIL : not owner cant add condition", async () => {
      let schemaHash = '9a7a6a513e29844f895cac0c6075ea77';
      await expect(cDroppin.connect(tay).addCondition(1,fromLittleEndian(hexToBytes(schemaHash)))).to.be.revertedWith("caller is not the owner of this group")
    });

    it("should be able to add 1 or more quests", async () => {
      let questInputDataFormat = ["bytes32","uint256","uint256","uint256"];
      let testCasesQuests = [[
      ...
      for (let i = 0; i<testCasesQuests.length;i++){
        let tx = await cDroppin.connect(pia).addQuest(testCasesQuests[i][4],defaultAbiCoder.encode(questInputDataFormat,testCasesQuests[i].slice(0,4)));
        await tx.wait();
      }

      for (let i = 0; i<testCasesQuests.length;i++){
        let rd = await cDroppin.questById(i+1);
