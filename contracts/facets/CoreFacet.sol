...
contract CoreFacet {
    ...
    function addQuest(LibCoreFacet.QuestData memory _questData) external {
        ...
        ds.questById[ds.questIds.current()].engagePoints = _questData.engagePoints;
    }
    ...
    function completeQuest(uint256 _questId) external {
        ...
        LibCoreFacet.CoreState storage ds = LibCoreFacet.diamondStorage();
        require(ds.isCompletedByUser[_questId][msg.sender] == false, "user has already completed this quest");
        ds.isCompletedByUser[_questId][msg.sender] = true;
        ds.userEngagePoints[ds.questById[_questId].groupId][msg.sender]+=(ds.questById[_questId].engagePoints);
    }
    function getGroup(uint256 _groupId) external view returns (LibCoreFacet.GroupData memory) {
        LibCoreFacet.CoreState storage ds = LibCoreFacet.diamondStorage();
        return (ds.groupById[_groupId]);
    }
    function getQuest(uint256 _questId) external view returns (LibCoreFacet.QuestData memory) {
        LibCoreFacet.CoreState storage ds = LibCoreFacet.diamondStorage();
        return (ds.questById[_questId]);
    } 
    function getUserEngagePoint(uint256 _groupId) external view returns(uint256){
        LibCoreFacet.CoreState storage ds = LibCoreFacet.diamondStorage();
